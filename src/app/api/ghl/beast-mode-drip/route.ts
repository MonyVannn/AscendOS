...
import {
  beastModeDripSchema,
  buildBeastModeDripPayload,
  BEAST_MODE_DRIP_LABEL,
} from "@/lib/ghl/beast-mode-drip";
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = beastModeDripSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const payload = buildBeastModeDripPayload(parsed.data);
    ...
    logSubmission({
      ...
      integration_key: "beast-mode-drip",
      template: BEAST_MODE_DRIP_LABEL,
      ...
      try {
        await fetchMutation(
          api.activityLog.recordSubmission,
          {
            agencyId: agency._id,
            userId: user._id,
            toolName: "beast-mode-drip",
            templateName: BEAST_MODE_DRIP_LABEL,
            contactEmail: parsed.data.email,
            contactName: `${parsed.data.first_name} ${parsed.data.last_name}`.trim(),
    ...
    console.error("Unexpected error in beast-mode-drip proxy:", error);
    return NextResponse.json(
